...
public class Driver
{
    ...
    {
        w.addPerson(new Person(1001, "255", "TA"));
        w.addPerson(new Person(1002, "Boby", "Jack"));
        w.addPerson(new Person(1003, "Sally", "Morgan"));
        w.addPerson(new Person(1004, "Waleed", "Nasr"));
    }
    ...
    public static void handlePrintMessages(Website w, int firstUID, int secondUID)
    {
        ...
        if (a == null)
        {
            System.out.println("Person with uid "+firstUID+
                    " not found, can't print message history");
        }

        Person b = w.getPersonByUID(secondUID);
        if (b == null)
        {
            System.out.println("Person with uid "+secondUID+
                    " not found, can't print message history");
        }

        System.out.println("Showing chat history between ["+a.getFirstName()+
                "] and ["+b.getFirstName()+"]");
        boolean any = false;
